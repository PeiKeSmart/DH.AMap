...
<head>
    ...
    <script src="https://webapi.amap.com/maps?v=2.0&key=任意字符串"></script>
</head>
<body>
    <div id="map"></div>
    <script>
...
export default {
  ...
  methods: {
    async initMap() {
      AMapLoader.load({
        key: "任意字符串",  // JS API 需要 key 参数，但 REST API 代理会自动添加 jscode
        version: "2.0",
...
- JavaScript API：加载地图 JS 文件时的 `key` 参数可以填写任意字符串（仅用于满足参数要求）
